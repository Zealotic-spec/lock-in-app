...
function formatDaysForPrompt(days: DayMetric[]): string {
  return days
    .map(
      d =>
        `${d.dayLabel}: [Analytics] ${d.focusMinutes} мин фокуса, ${d.sessions} сессий таймера · [Tracking] ${d.tasksCompleted} привычек/задач отмечено`
    )
    .join("\n");
}
...
export async function generateWeeklyAnalysis(weekKey: string, days: DayMetric[]): Promise<string> {
  ...
  const prompt = [
    ...
    "Ты анализируешь ДВА раздела сразу: вкладку Analytics (реальные сессии фокус-таймера —",
    "минуты и количество сессий) и вкладку Tracking (еженедельные привычки/задачи с чекбоксами",
    "по дням). Связывай их между собой — например, замечай дни, где много отметок в Tracking,",
    "но мало фокус-времени в Analytics, или наоборот.",
    ...
    "1. Назови 2-3 сильные стороны недели (опираясь и на Analytics, и на Tracking).",
...
export async function generateMonthlyAnalysis(
  ...
  const prompt = [
    "Ты — персональный AI-коуч продуктивности в приложении LOCK IN.",
    "Каждый из еженедельных анализов ниже уже объединяет данные вкладки Analytics",
    "(фокус-сессии таймера) и вкладки Tracking (еженедельные привычки/задачи).",
